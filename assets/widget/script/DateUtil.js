...
var DateUtil = {
    ...
    countDays: function(date1, date2) {
        ...
        if (date1 instanceof Date && date2 instanceof Date) {
            date1 = DateUtil.formatDate(date1, fmt);
            date2 = DateUtil.formatDate(date2, fmt);
        }
        if (typeof date1 === 'string' && typeof date2 === 'string') {
            date1 = DateUtil.parseDate(date1, fmt);
            date2 = DateUtil.parseDate(date2, fmt);
     ...
     * @param date2 日期二
     */
    countSeconds: function(date1, date2) {
        var fmt = 'yyyy-MM-dd HH:mm:ssa';
        // 将日期转换成字符串，转换的目的是去除“时、分、秒”
        if (date1 instanceof Date && date2 instanceof Date) {
            date1 = DateUtil.formatDate(date1, fmt);
            date2 = DateUtil.formatDate(date2, fmt);
